...
async function loginUser(email, password) {
  ...
  if (!hasMatchedPassword)
    throw createHttpError(422, "User's email or password is not correct");
...
const updatePassword = async (user, payload) => {
  ...
  if (password === new_password)
    throw createHttpError(422, "new_password can't match password");

  const hasValidOldpassword = await bycrpt.compare(password, user.password);
  if (!hasValidOldpassword)
    throw createHttpError(422, "Entered information is not valid");
